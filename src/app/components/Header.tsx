// components/Header.tsx (UI FIXED VERSION - WITH PAGINATION & MEMOIZED CLICKS)
"use client";
...
const Header: React.FC = () => {
  ...
  const extractPaginationInfo = useCallback((data: any) => {
    try {
      if (!data?.notifications?.pageInfo) return null;
  ...
  // OPTIMIZED: Use Apollo's built-in polling with pagination support
  const { 
    ...
    pollInterval: userId && !isBellPopupOpen ? 10000 : 0, // Stop polling when popup is open
    onError: (error) => {
      console.error('Notification query error:', error);
    },
    onCompleted: (data) => {
      // Handle new notifications when query completes (only for first page)
      if (data && userId && !currentCursor) {
  ...
  const totalPages = useMemo(() => {
    return Math.ceil(pagination.totalCount / 5);
  ...
  }, [pagination.hasNextPage, pagination.endCursor, pagination.currentPage, userId, refetchNotifications]);

  const handlePreviousPage = useCallback(async () => {
    if (!pagination.hasPreviousPage || !pagination.startCursor) return;
  ...
  }, [pagination.hasPreviousPage, pagination.startCursor, pagination.currentPage, userId, refetchNotifications]);
  ...
  const handleLastPage = useCallback(async () => {
    // This would require knowing the last cursor, which might need a separate query
    // For now, we'll just show a message or implement a "load more" approach
    console.log('Last page navigation requires additional implementation');
  }, []);
  ...
  // Refetch notifications when bell popup opens (reset to first page)
  useEffect(() => {
  ...
  }, [isBellPopupOpen, userId, refetchNotifications, currentCursor]);
  ...
  // MEMOIZED: Handle notification click without refetching
  const handleNotificationClick = useCallback(async (notification: Notification) => {
    // Don't refetch notifications - just handle the click action
  ...
  // Memoize pagination controls to prevent re-renders
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50">
        <div className="flex items-center space-x-2">
          <button
            ...
            className="p-1 text-gray-500 hover:text-purple-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
            title="First page"
          >
  ...
  }, [pagination.currentPage, pagination.hasPreviousPage, pagination.hasNextPage, totalPages, pagination.totalCount, handleFirstPage, handlePreviousPage, handleNextPage, handleLastPage]);
          ...
          <div className="z-20 h-[100%] flex items-center space-x-2">
            ...
            {!isAuthPage && (
              <div className="relative" ref={bellRef}>
                ...
                {/* Slide-up Bell Popup with enhanced animation and pagination */}
