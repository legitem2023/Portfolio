...
import { QrCode, Star, MapPin, X } from "lucide-react";
...
function SupplierOrderCard({ group, onSelect, onTrackOrder }: { 
  group: SupplierGroup; 
  onSelect: () => void;
...
function SupplierOrderModal({ group, onClose, onWriteReview, onTrackOrder }: { 
  group: SupplierGroup; 
  onClose: () => void;
          ...
          <div className="flex gap-2 pt-2">
            {riderId && isShipped && deliveryAddress ? (
              ...
              <button className="flex-1 bg-purple-600 text-white py-2.5 rounded-lg font-medium text-sm hover:bg-purple-700 transition-colors">
                {isDelivered ? "View Delivery Details" : "Order Processing"}
              </button>
            )}
