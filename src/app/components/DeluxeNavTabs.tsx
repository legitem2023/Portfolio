...
const DeluxeNavTabs: React.FC = () => {
  ...
  // Refs and state for slider functionality
  ...
  useEffect(() => {
    checkArrows();
    if (tabsContainerRef.current) {
      tabsContainerRef.current.addEventListener('scroll', checkArrows);
    }
    return () => {
      if (tabsContainerRef.current) {
        tabsContainerRef.current.removeEventListener('scroll', checkArrows);
      }
    };
  }, []);
    ...
    <div className="w-full max-w-6xl mx-auto font-sans z-10">
      ...
      <div className="relative flex items-center mb-1 bg-gradient-to-t from-violet-50 to-white z-20">
        ...
        <div
          ...
          className="flex overflow-x-auto hide-scrollbar w-full py-2 px-8"
        >
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => handleTabClick(tab.id)}
              className={`flex-shrink-0 flex items-center px-5 py-3 text-lg font-large whitespace-nowrap transition-all duration-300 border-b-4 mx-1 ${
                activeIndex === tab.id
                  ? 'border-indigo-800 text-indigo-800 bg-gradient-to-t from-indigo-50 to-white'
                  : 'border-violet-600 text-violet-600 hover:text-violet-600 hover:border-violet-400'
              }`}
