...
const MerchantDetails = ({ userId }: { userId: string }) => {
  ...
  // Optional: Add validation for required user data
  if (!user.firstName || !user.lastName) {
    return <div className="min-h-screen flex items-center justify-center text-red-500">Invalid user data</div>;
  }
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div className="w-full md:w-auto">
              ...
              <div className="flex gap-4 md:gap-6 mt-4 text-gray-700 text-sm md:text-base">
                ...
                <span className="hover:underline cursor-pointer">
                  <strong>{user.products?.length || 0}</strong> Products
                </span>
              </div>
            </div>
