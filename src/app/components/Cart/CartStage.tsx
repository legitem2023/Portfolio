...
const CartStage = ({ cartItems, subtotal, shippingCost, tax, total, onQuantityChange, onCheckout }: CartStageProps) => {
  ...
  return (
    <div>
      <h2 className="text-2xl font-serif font-bold text-amber-900 mb-6 pb-2 border-b border-amber-100">Your Shopping Cart</h2>
      <div className="mb-8">
        {cartItems.map(item => (
          <div key={item.id} className="flex flex-col md:flex-row items-center py-5 border-b border-amber-50 hover:bg-amber-50 transition-all duration-200 rounded-lg">
            <img src={item.image} alt={item.name} className="w-24 h-24 object-cover rounded-lg border border-amber-200 shadow-sm" />
            <div className="flex-1 px-4 md:px-6 py-2">
              <h3 className="font-serif font-semibold text-lg text-amber-900">{item.name}</h3>
              <p className="text-amber-600 text-sm">{item.description}</p>
              <div className="text-amber-700 font-medium mt-1">${item.price.toFixed(2)}</div>
            </div>
            <div className="flex items-center space-x-3 my-4 md:my-0">
              ...
              <span className="font-medium text-amber-900 w-6 text-center">{item.quantity}</span>
              <button 
                onClick={() => onQuantityChange(item.id, item.quantity + 1)}
                className="w-8 h-8 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center hover:bg-amber-200 transition-colors"
              >
            ...
            <div className="font-semibold text-amber-900 min-w-[80px] text-right px-4">
      ...
      <div className="bg-amber-50 rounded-xl p-6 border border-amber-200">
        ...
        <button 
          className="w-full bg-gradient-to-r from-amber-500 to-amber-600 text-white py-3 rounded-lg font-semibold mt-4 hover:from-amber-600 hover:to-amber-700 transition-all shadow-md hover:shadow-lg"
          onClick={onCheckout}
        >
          Proceed to Checkout
        </button>
