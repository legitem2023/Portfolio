...
const BASE_RATE = process.env.NEXT_PUBLIC_BASE_RATE;
const RATE_PER_KM = process.env.NEXT_PUBLIC_RATE_PER_KM;
          ...
          <section>
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              {Array.from(supplierGroups.entries()).map(([supplierId, items]) => {
                ...
                const itemsPerSupplier = items.length;
                const shippingForSupplier = shippingCostPerSupplier;
                const shippingPerItem = itemsPerSupplier > 0 ? shippingForSupplier / itemsPerSupplier : 0;
                
                return (
                  <div key={supplierId} className="mb-4 last:mb-0">
                    <div className="bg-indigo-50 p-2 rounded-t-lg border-b border-indigo-200">
                      <div className="flex justify-between items-center">
                        ...
                        <div className="text-right">
                          <p className="text-xs text-indigo-700">Shipping for this supplier:</p>
                          <p className="text-sm font-semibold text-indigo-900">{formatPesoPrice(shippingForSupplier)}</p>
                          <p className="text-xs text-indigo-600">({formatPesoPrice(shippingPerItem)} per item)</p>
                        </div>
                      </div>
                    </div>
                    
                    <div className="space-y-2 p-2 bg-white rounded-b-lg border border-gray-200 border-t-0">
                      {items.map((item: any) => {
        ...
        <div className="bg-indigo-50 rounded-lg p-3">
          ...
          <div className="flow-root">
            <dl className="-my-2 text-xs divide-y divide-indigo-200">
              ...
              {showShippingBreakdown && supplierCount > 1 && (
                <div className="py-2">
                  <dt className="text-indigo-700 mb-1">Shipping Breakdown:</dt>
                  {Array.from(supplierGroups.entries()).map(([supplierId, items]) => {
                    const supplierItemCount = items.length;
                    const shippingForSupplier = shippingCostPerSupplier;
                    return (
                  ...
                  <div className="flex items-center justify-between text-xs pl-2 mt-1 pt-1 border-t border-indigo-200">
                    <span className="text-indigo-600 font-medium">Total Shipping:</span>
                    <span className="font-medium text-indigo-900">{formatPesoPrice(shippingCost)}</span>
                  </div>
                </div>
              )}
              ...
              <div className="py-2 flex items-center justify-between">
                <dt className="text-indigo-700">Tax (8%)</dt>
...
const DeluxeCart = () => {
  ...
  const handlePlaceOrder = async (itemDistances?: Record<string, number>): Promise<void> => {
    ...
    try {
      ...
      const shippingCostPerSupplier = supplierCount > 0 ? calculatedShippingCost / supplierCount : 0;
      ...
      console.log('Placing order with supplier-based shipping distribution:', {
        totalShippingCost: calculatedShippingCost,
        supplierCount,
        shippingCostPerSupplier,
        orderItems: orderItems.map(item => ({
          productId: item.productId,
          supplierId: item.supplierId,
          individualShipping: item.individualShipping_input
        }))
      });
      ...
      if (result.data?.createOrder?.statusText === 'Order Successful!') {
        ...
        throw new Error(result.data?.createOrder?.statusText || 'Failed to create order');
      }
    } catch (err) {
