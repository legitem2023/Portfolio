...
import VariantsCollapsible from './Products/VariantsCollapsible';
...
export default function ProductTable({ products,refetch, onProductDeleted }: ProductTableProps) {
  ...
  const openVariantsCollapsible = (product: Product) => {
    if (selectedProduct?.id === product.id && isCollapsibleOpen) {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredAndSortedProducts.map((product) => (
              <React.Fragment key={product.id}>
                <TableRow 
                  product={product} 
                  onViewVariants={openVariantsCollapsible}
                  onImageUpload={handleProductImageUpload}
                  onDeleteProduct={handleDelete}
                  isUploading={uploadingProductId === product.id}
                  isVariantsOpen={selectedProduct?.id === product.id && isCollapsibleOpen}
                />
                {selectedProduct?.id === product.id && isCollapsibleOpen && (
