...
export default function AddVariantForm({ 
  ...
  if (foodCatError && isFoodsAndDrinks()) {
    return (
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6 sm:space-y-8">
              ...
              <div>
                ...
                {isFoodsAndDrinks() ? (
                  <div className="space-y-3">
                    ...
                    {selectedCategory && (
                      <div>
                        <label className="block text-sm text-gray-600 mb-1">Food Item (Flavor)</label>
                        <select
                          className="w-full px-4 py-2.5 text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                          value={selectedItem}
                          onChange={(e) => {
                            ...
                            setFormData({
                              ...formData, 
                              name: selectedItemObj?.name || itemId,
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Selected item becomes both Variant Name and Flavor
                        </p>
                      </div>
                    )}
                  </div>
        ...
        {/* Color Section - Only show for non-Foods and Drinks */}
        {!isFoodsAndDrinks() && (
          <div className="pt-2 border-t border-gray-100">
            <label className="block text-sm font-semibold text-gray-700 mb-4">
              Color Selection
        ...
        <div className="pt-2 border-t border-gray-100">
          <label className="block text-sm font-semibold text-gray-700 mb-4">
            {isFoodsAndDrinks() ? 'Size/Portion' : 'Size Configuration'}
          </label>
          <div className="space-y-4">
            {isFoodsAndDrinks() ? (
              <input
                ...
                name="size"
                className="w-full px-4 py-2.5 text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 hover:border-gray-400"
                placeholder="Enter portion size (e.g., 12oz, 500ml, Large, Family Size)"
                value={formData.size || ''}
                onChange={handleChange}
              />
            ) : (
