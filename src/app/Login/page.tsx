...
export default function LuxuryLogin() {
  ...
  // 🆕 SESSION EXPIRATION CHECK - Check every 30 seconds and on visibility change
  useEffect(() => {
    let mounted = true;
    ...
    const checkSessionExpiration = async () => {
      ...
      try {
        ...
        if (session) {
          // Check if the session has an expiration field
    ...
    intervalId = setInterval(checkSessionExpiration, 30000);
    ...
    // Check before unload (user leaving page)
  ...
  // 🆕 Handle manual logout (for testing or user-initiated)
        ...
        <div className="flex items-center justify-center py-6 sm:py-8 md:py-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl border border-indigo-100">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-medium text-blue-900">{userData.name || userData.email || userData.phone}</p>
                    <span className={`inline-block px-2 py-1 mt-1 text-xs font-semibold rounded-full border ${getRoleBadgeColor(userData.role)}`}>
                      {userData.role}
                    </span>
                  </div>
                  <button
            ...
            {!userData || isSessionExpired ? (
              ...
              // 🆕 Show continue button when already logged in
              <div className="mt-6">
                <button
                  onClick={handleContinue}
                  className="w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-all shadow-lg shadow-green-500/20"
                >
                  Continue to {getRedirectPath(userData?.role || 'USER')}
                </button>
              </div>
            )}
