...
export default function LuxuryLogin() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session?.serverToken) {
      ...
      // setTimeout(() => setShowTokenInfo(false), 10000);
    }
  }, [session, status]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setIsLoading(true);
    setShowTokenInfo(false); // Hide previous token info
      ...
      <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-violet-50 p-0">
        ...
        <div className="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl border border-indigo-100">
            ...
            {/* Token Info Display - Shows after successful login */}
            {showTokenInfo && (
              <div className="bg-gray-50 border border-indigo-200 rounded-lg p-4 mb-4">
                <h3 className="text-lg font-semibold text-indigo-800 mb-2">🔐 Login Successful - Token Info</h3>
                
                {/* Raw Token */}
                <div className="mb-3">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Raw Server Token:</p>
                ...
                {/* Decrypted Token */}
                <div className="mb-3">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Decrypted Token:</p>
                  {tokenData.error ? (
                    <div className="bg-red-50 p-2 rounded border border-red-200">
                      <p className="text-xs text-red-600">❌ Decryption failed: {tokenData.error}</p>
                    </div>
                  ) : (
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-white p-2 rounded border border-gray-200">
                    <span className="font-medium">Provider:</span> {session?.provider || 'N/A'}
                  </div>
                  <div className="bg-white p-2 rounded border border-gray-200">
            ...
            {/* Login Form - Hide when showing token info */}
            {!showTokenInfo && (
            ...
            {/* Divider - Hide when showing token info */}
            {!showTokenInfo && (
              <div className="relative">
            ...
            {/* Social Login - Hide when showing token info */}
            {!showTokenInfo && (
