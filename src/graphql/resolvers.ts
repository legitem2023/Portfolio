...
getReturnStats: async (_: any, { userId }: any) => {
  try {
    ...
    const totalRefund: any = await prisma.returnRequest.aggregate({
      where: { ...where, status: 'COMPLETED' },
      _sum: { refundAmount: true }
    });
    
    const completedReturns: any[] = await prisma.returnRequest.findMany({
      where: { ...where, status: 'COMPLETED' },
      select: { createdAt: true, updatedAt: true }
    });
    ...
    if (completedReturns.length > 0) {
      ...
      for (const ret of completedReturns) {
        const createdAt: any = ret.createdAt;
        const updatedAt: any = ret.updatedAt;
        const days = (new Date(updatedAt).getTime() - new Date(createdAt).getTime()) / (1000 * 60 * 60 * 24);
    ...
    const allReturns: any[] = await prisma.returnRequest.findMany({
      where: {
        ...where,
        createdAt: { gte: sixMonthsAgo }
      },
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const monthReturns = allReturns.filter((r: any) => {
        const rDate = new Date(r.createdAt);
        return rDate >= startOfMonth && rDate <= endOfMonth;
      });
      ...
      for (const r of monthReturns) {
        monthTotal = monthTotal + (r.refundAmount || 0);
      }
...
getReturnReasons: async (_: any, { category }: any) => {
  try {
    const where: any = { isActive: true };
    if (category) where.category = category;
...
// Define the enum values that match your Prisma schema
 ...
 riderPayments: async(parent: any, args: any) => {
  ...
  refundedorder: async(parent: any, args: any) => {
  // Set default values
  const filter = args.filter || {};
  const pagination = args.pagination || {};
  ...
  returnorder: async(parent: any, args: any) => {
  // Set default values
  const filter = args.filter || {};
  const pagination = args.pagination || {};
...
//===========
salesorder: async(parent: any, args: any) => {
  // Set default values
  const filter = args.filter || {};
  const pagination = args.pagination || {};
...
/*products: async (
  ...
  try {
    // FOR TESTING: Simulate 10 million records
    // Comment out this block when you want to use real data
    if (limit > 1000) { // Trigger test mode when requesting large limits
      console.log('⚠️ TEST MODE: Simulating 10 million records for performance testing');
      ...
      // Generate items for current page by repeating the template
      const items = [];
      const endIndex = Math.min(startIndex + limit, TOTAL_RECORDS);
      
      for (let i = startIndex; i < endIndex; i++) {
        // Create a deep copy with unique ID
        items.push({
          ...realProduct,
          id: `test-${i}`, // Unique ID based on index
          name: `${realProduct.name} (Copy ${i + 1})`, // Slightly modify name to track copies
          sku: `${realProduct.sku}-COPY-${i}`,
          // Keep timestamps varied to simulate real data
          createdAt: new Date(Date.now() - (i * 60000)), // Different timestamps
          updatedAt: new Date(Date.now() - (i * 30000)),
          // Ensure supplier and category are deeply copied
          supplier: realProduct.supplier ? {
            addresses: realProduct.supplier.addresses ? [...realProduct.supplier.addresses] : []
          } : null,
          category: realProduct.category ? {
            ...realProduct.category,
            id: realProduct.category.id,
            name: realProduct.category.name
          } : null,
          variants: realProduct.variants ? realProduct.variants.map(v => ({
            ...v,
            id: `var-${i}-${v.id}`, // Unique variant IDs
            sku: `${v.sku}-COPY-${i}`
          })) : []
        });
      }
      ...
      console.log(`✅ Returning page ${Math.floor(startIndex / limit) + 1}: ${items.length} items (total simulated: ${TOTAL_RECORDS})`);
...
/*createReturn: async (_: any, { input }: any) => {
  try {
    ...
    if (!finalSupplierId) {
      ...
      finalSupplierId = order?.items[0]?.supplierId;
    }
...
updateReturnStatus: async (_: any, { input }: any) => {
  try {
    ...
    switch (status) {
      case 'APPROVED':
        notificationTitle = 'Return Request Approved';
        notificationMessage = `Your return request #${existingReturn.returnNumber} has been approved. Please ship the items back.`;
        break;
      case 'REJECTED':
        notificationTitle = 'Return Request Rejected';
        notificationMessage = `Your return request #${existingReturn.returnNumber} has been rejected. ${vendorNotes || 'Please contact support for more information.'}`;
        break;
      case 'RECEIVED':
        notificationTitle = 'Return Items Received';
        notificationMessage = `We have received your returned items for request #${existingReturn.returnNumber}. They are being inspected.`;
        break;
      case 'REFUND_INITIATED':
        notificationTitle = 'Refund Initiated';
        notificationMessage = `Refund for return #${existingReturn.returnNumber} has been initiated.`;
        break;
      case 'COMPLETED':
        notificationTitle = 'Return Completed';
        notificationMessage = `Return #${existingReturn.returnNumber} has been completed. Refund has been processed.`;
        break;
      default:
        notificationTitle = 'Return Status Updated';
        notificationMessage = `Your return request #${existingReturn.returnNumber} status has been updated to ${status}`;
    }
    ...
    if (existingReturn.status === 'APPROVED') {
      ...
      await prisma.notification.create({
        data: {
          ...
          type: 'RETURN_SHIPPED',
          title: 'Return Items Shipped',
          message: `Customer has shipped return #${existingReturn.returnNumber}. Tracking: ${trackingNumber}`,
          link: `/supplier/returns/${returnId}`,
          returnId
        }
      });
  ...
  try {
    ...
    const resolution = await prisma.returnResolution.create({
      data: {
        returnId,
        resolution: refundMethod === 'STORE_CREDIT' ? 'STORE_CREDIT' : 
                    refundAmount === existingReturn.refundAmount ? 'FULL_REFUND' : 'PARTIAL_REFUND',
        partialAmount: refundAmount !== existingReturn.refundAmount ? refundAmount : null,
        adminNotes: `Refund processed via ${refundMethod}`,
        resolvedBy: resolvedBy
      }
    });
    ...
    await prisma.notification.create({
      data: {
        ...
        type: 'REFUND_PROCESSED',
        title: 'Refund Processed',
        message: `Your refund of ${refundAmount} for return #${existingReturn.returnNumber} has been processed via ${refundMethod}. Transaction ID: ${transactionId || 'N/A'}`,
        link: `/account/returns/${returnId}`,
        returnId
      }
    });
    
    // Don't use setTimeout in serverless, just return
    return {
      ...returnRequest,
      resolution
    };
...
cancelReturn: async (_: any, { input }: any) => {
  try {
    ...
    if (existingReturn.status !== 'PENDING' && existingReturn.status !== 'APPROVED') {
      throw new Error('Return cannot be cancelled at this stage');
    }
    ...
    await prisma.notification.create({
      data: {
        ...
        type: 'RETURN_STATUS_UPDATED',
        title: 'Return Cancelled',
        message: `Return request #${existingReturn.returnNumber} has been cancelled. Reason: ${reason || 'Not specified'}`,
        link: `/returns/${returnId}`,
        returnId
      }
    });
  ...
  try {
    ...
    const addressResponse = await prisma.address.create({
      data: {
        ...
        phone: phone,
        street: businessStreet || addressInstruction,
        city: businessCity,
        state: businessState,
  ...
  try {
    ...
    if (input.productId && input.productId !== existingVariant.productId) {
      ...
      if (!input.productId.trim()) {
        return {
          statusText: 'Product ID cannot be empty'
        };
      }
    ...
    // Always update the parent product if the variant has a productId
    ...
    if (input.productId !== undefined) {
      // Handle productId update properly
      if (input.productId === null || input.productId === '') {
        // If you want to allow removing the product association
        ...
        if (!targetProduct) {
          return {
            statusText: 'Target product not found'
          };
        }
        updateData.productId = input.productId;
      } else {
        return {
          statusText: 'Invalid product ID format'
        };
      }
    }
    if (input.sku !== undefined) updateData.sku = input.sku;
...
if (activeorder.length > 0) {
  return {
    statusText: "Changing Default Address while there's an active transaction is not allowed!",
    token: "",
    role: ""
  }
}                                                    
    ...
    if (response) {
      try {
        ...
        const notificationPromises = Object.entries(itemsBySupplier).map(async ([supplierId, supplierItems]: [string, any]) => {
          // Build a message listing all items for this supplier
          const itemList = supplierItems.map((item: any) => 
            `${item.quantity}x ${item.productId}` // You can replace with product name if available
          ).join(', ');
      ...
      try {
        ...
        await Promise.all(items.map(async (item: any) => {
          await createNotification({
            ...
            type: NotificationType.ORDER_CREATED,
            title: "Order Created Successfully",
            message: `Order #${response.orderNumber} has been created for item ${item.productId}`,
            link: ``,
            isRead: false
          });
        }));
  ...
  try {
    ...
    const validItems = items.map((item: any) => {
      ...
      return {
        ...
        individualDistance: item.individualDistance_input || 0, // Add this field
        status: "PENDING",
      };
    });
